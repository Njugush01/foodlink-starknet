...
export default function PendingListings() {
  ...
  doc.text(titleText, doc.internal.pageSize.width / 2, 45, {
    ...
    doc.save("pending_listings.pdf");
  };

  return (
    <div>
      <h1 className="font-bold text-2xl">Pending Listings</h1>
      ...
      {loading && <p>Loading...</p>}
      {!loading && pendingListings && pendingListings.length > 0 && (
        <div>
          <table>
            <thead>
              <tr>
      ...
      {selectedDate &&
        pendingListings.every(
          (listing) =>
            new Date(listing.created_at).getFullYear() !==
              selectedDate.getFullYear() ||
            new Date(listing.created_at).getMonth() !== selectedDate.getMonth()
        ) && <p>No pending listings available for selected month</p>}
    </div>
  );
}
