...
export default function AcceptedListings() {
  ...
  const getAcceptedListings = () => {
    setLoading(true);
    axiosClient
      .get("/auth/accepted")
      .then(({ data }) => {
        setLoading(false);
        setAcceptedListings(data);
      })
      .catch(() => {
        setLoading(false);
      });
  ...
  const handlePDFDownload = () => {
    ...
    doc.save("accepted_listings.pdf");
  };

  return (
    <div>
      <h1 className="font-bold text-2xl">Accepted Listings</h1>
      ...
      {loading && <p>Loading...</p>}
      {!loading && acceptedListings && acceptedListings.length > 0 && (
        <div>
          <table>
            <thead>
              <tr>
      ...
      {selectedDate &&
        acceptedListings.every(
          (listing) =>
            new Date(listing.created_at).getFullYear() !==
              selectedDate.getFullYear() ||
            new Date(listing.created_at).getMonth() !== selectedDate.getMonth()
        ) && <p>No accepted listings available for selected month</p>}
    </div>
  );
}
